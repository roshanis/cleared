...
import { currentQueue, currentRun, runHref } from "@/lib/review-workspace";
...
export default async function QueuePage({ searchParams }: { searchParams: Promise<{ q?: string; priority?: string; sort?: string; scope?: string; page?: string }> }) {
  ...
  const base = query.scope === "all" ? reviewQueue(db) : current;
  ...
  return <div className="space-y-6">
    ...
    <Card className="overflow-hidden">
      ...
      <ul className="divide-y divide-line">{rows.slice((page-1)*25, page*25).map(({ document, run, version }) => <li key={run.id} className="grid items-center gap-4 p-5 transition-colors hover:bg-rail/40 lg:grid-cols-[1fr_12rem_10rem]">
        ...
        <div>{run.result && <VerdictBadge verdict={run.result.verdict} />}</div>
        <div className="flex items-center justify-between gap-3 lg:block"><p className="text-xs text-muted">Submitted {relativeTime(run.createdAt)}</p><Link href={runHref(document.id,run.id)} className={`${buttonClass("secondary", "sm")} lg:mt-2`}>Review →</Link></div>
      </li>)}</ul>
    </Card>}
