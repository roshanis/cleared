...
let osDefaults: { media: string; content: string }[] | null = null;

/**
 * iOS Safari tints its toolbar from `theme-color`. The two tags in the layout
 * answer to the OS, so an explicit choice here has to be pushed to them or the
 * toolbar keeps the other scheme and frames the page in the wrong colour.
 * The value is read back off the header — the surface the toolbar actually
 * meets — rather than repeating a hex that could drift from the palette.
 */
function syncBrowserChrome(choice: ThemeChoice) {
  ...
  // Captured before the first override so "system" can hand the toolbar back
  // to the OS, media conditions and all.
  osDefaults ??= tags.map((tag) => ({
    media: tag.media,
    content: tag.content,
  }));
  ...
  if (choice === "system") {
    tags.forEach((tag, index) => {
      const original = osDefaults?.[index];
      if (!original) return;
      tag.media = original.media;
      tag.content = original.content;
    });
    return;
  }
  ...
  if (!header) return;
  // Drop the media conditions: the page is no longer following the OS.
...
export function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    setChoice(resolved);
    setReady(true);
    // The pre-paint script sets the theme but cannot see the meta tags yet.
      ...
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={`touch-segment inline-flex h-6 w-7 items-center justify-center rounded transition-colors duration-150 ${
              active
